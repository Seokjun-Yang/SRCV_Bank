...
#from SRCV_Bank-master.screens.image_button import ImageButton
...
class SignupVerification(Screen):
    ...
    def capture_image(self, instance):
        ...
        if texture:
            try:
                ...
                manager.save_temp_metadata(self.image_name)
                #threading.Thread(target=self.speak, args=(self.status_label.text,)).start()
                time.sleep(3)
    ...
    def delete_registered_image(self, instance=None):
        """저장된 이미지를 삭제하는 함수"""
        #self.storage_path
        registered_image_path = self.image_path#os.path.join(self.storage_path, )
        #= os.path.join(self.storage_path, )  # os.path.join(self.storage_path, 'registered_image.jpg')
