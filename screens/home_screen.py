...
class HomeScreen(Screen):
    ...
    def setup_ui(self):
        ...
        self.welcome_label = Label(text="환영", font_name=fontName1, font_size=19, color=(1, 1, 1, 1), halign='left',
                                   valign='middle',
        ...
        self.account_number_label = Label(text="계좌번호", font_name=fontName1, font_size=19, color=(1, 1, 1, 1),
    ...
    def refresh_balance(self):
        ...
        if self.user_seq_no:
            ...
            if account_data:
                ...
                self.balance_label.text = f'{balance_amt:,}원'
                self.balance_shadow_label.text = f'{balance_amt:,}원'
                #App.get_running_app().speak_text(f'잔액이 {balance_amt}원으로 업데이트 되었습니다.')
            else:
    ...
    def on_transfer_button_pressed(self, instance):
        ...
        if self.sender_account:
            ...
            #카메라 화면으로 이동
            App.get_running_app().speak_text("송금 버튼을 눌렀습니다. 본인 확인을 위해 카메라 화면으로 이동합니다.")
            Clock.schedule_once(lambda dt:self.change_screen('transferVerification'), 5)

        else:
            print("송금할 계좌가 없습니다.")  # 오류 메시지 출력
            App.get_running_app().speak_text("송금할 계좌가 없습니다.")
    ...
    def on_enter(self, *args):
        App.get_running_app().speak_text(self.welcome_label.text)
        Clock.schedule_once(lambda dt:App.get_running_app().delay, 1.5)
        App.get_running_app().speak_text(f'현재 잔액은 {self.balance_label.text}입니다.')
        Clock.schedule_once(lambda dt:App.get_running_app().delay, 1.5)
        pass
